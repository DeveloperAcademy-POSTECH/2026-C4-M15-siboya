...
- 새로 작성하거나 수정하는 모든 코드에는 작성 의도와 역할을 이해할 수 있는 한국어 주석을 작성합니다.
- 타입에는 담당 책임, 프로퍼티에는 보관하는 값, 함수에는 입력·처리·결과를 설명하는 주석을 작성합니다.
- 화면 코드에는 주요 레이아웃 영역과 사용자 상호작용이 어떤 역할을 하는지 주석으로 구분합니다.
- 조건 분기, 상태 변경, 데이터 변환처럼 흐름을 이해하는 데 필요한 핵심 처리 단계에는 이유를 설명하는 주석을 작성합니다.
- 테스트에는 검증하려는 상황과 기대 결과를 설명하는 주석을 작성합니다.
- 코드만 읽어도 명백한 내용을 그대로 반복하는 주석은 피하고, 구현이 변경되면 관련 주석도 함께 갱신합니다.
