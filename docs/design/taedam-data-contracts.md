...
> **2026-07-21 코드 확인**: `develop`에는 SwiftData 모델·Repository가 병합되어 있다. 태담 대본 진행은 `feature/SCRUM-23-taedam-screen-flow`, 결과 화면은 미병합 PR #6에 있다. Home·대본 미리보기·준비자세 View는 아직 구현 전이다.
...
- Home 추천 설명은 현재 `아빠의 낮은 목소리가 잘 들리는 시기`로 하드코딩하고, 추후 주차별 추천 설명 JSON 필드가 추가되면 데이터 값으로 교체한다.
