...
- 우측 상단 닫기는 iOS 26 `glassEffect`와 `xmark`를 사용하고 `Circle` shape로 원형을 명시한다.
- 닫기 버튼은 콘텐츠 흐름과 분리된 top-trailing overlay로 배치하며 sheet의 상단과 우측에서 각각 `16pt` 떨어진다.
- `36×36pt` symbol container를 정확한 `44×44pt` 버튼 영역 가운데에 두고 그 영역에 glass effect를 직접 적용한다. `.buttonStyle(.glass)`의 가변 기본 inset에는 의존하지 않는다.
...
9. 닫기 버튼은 `44×44pt` 원형 glass로 렌더링되고 sheet 상단·우측에서 각각 `16pt` 떨어져야 한다.
